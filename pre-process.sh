...
refSLR138="../DBs/SILVA/SILVA.138.1.LSU.fasta"
refSSR138="../DBs/SILVA/SILVA.138.1.SSU.fasta"
...
#Execute TrimGalore. Output files: $SampleName"_1_val_1.fq.gz" $SampleName"_2_val_2.fq.gz" $SampleName"_1_trimmed.fq.gz" $SampleName"_2_trimmed.fq.gz" 
trim_galore -j $JTrim --length 80 --paired $R1 $R2 -q 30 --fastqc > $Trimgalore 2>&1
echo -e "$(date) Trim Galore session finished \n" >> $logfile 2>&1

echo "List process files"

##Gunzip fq.gz paired files
echo -e "$(date) Uncompressing files \n" >> $logfile 2>&1

#Command to execute
gzip -d *val_*fq.gz # gunzip the TrimGalore output

##Rename files
echo -e "$(date) Renaming files \n" >> $logfile 2>&1
