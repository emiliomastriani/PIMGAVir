...
ConcScript="/usr/share/NGS-PKGs/scripts/concatenate_reads.py"
...
vsearch --derep_fulllength Combined.fasta --output derep.fasta --sizein --sizeout --uc combined.uc  --fasta_width 0 --threads $JTrim

echo "7. Perform Pre-Clustering (vsearch)"
echo -e "$(date) Perform Pre-Clustering (vsearch)" >> $logfile 2>&1
#Command to execute
## Perform Chimera Filter Denovo
vsearch --cluster_size derep.fasta --id 0.95 --sizein --sizeout --fasta_width 0 --centroids preclustered.fasta --threads $JTrim

echo "8. Perform Chimera Filter Denovo (vsearch)"
echo -e "$(date) Perform Chimera Filter Denovo (vsearch)" >> $logfile 2>&1
#Command to execute
vsearch --uchime_denovo preclustered.fasta --sizein --sizeout --fasta_width 0 --nonchimeras nonchimeras.fasta
...
vsearch --cluster_size nonchimeras.fasta --id 0.95 --sizein --sizeout --fasta_width 0 --uc clustered.uc --relabel OTU_ --centroids otus.fasta --otutabout otutab.txt --biomout otu.biom --msaout MSA.fa --threads $JTrim
